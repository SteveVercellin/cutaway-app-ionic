...
/*
  Generated class for the StripePaymentProvider provider.

  See https://angular.io/guide/dependency-injection for more info on providers
  and Angular DI.
*/
@Injectable()
export class StripePaymentProvider extends BaseRestProvider {
  ...
  getNoncePay()
  {
    if (this.authenticatedData.hasOwnProperty('token') && this.authenticatedData.token != '') {
      ...
      params = params.set('method', 'stripe');
  ...
  {
    if (this.authenticatedData.hasOwnProperty('token') && this.authenticatedData.token != '') {
      let serviceCall = this.libFunc.completeHttpService(ServicesConfig.payments.stripe.pay);
  ...
  {
    if (this.authenticatedData.hasOwnProperty('token') && this.authenticatedData.token != '') {
      let serviceCall = this.libFunc.completeHttpService(ServicesConfig.payments.stripe.refund);
